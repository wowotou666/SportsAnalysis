...
library(data.table)
df_t=transpose(df)
df_t=cbind(team_name,df_t)

colnames(df_t)=c("team","2000":"2020")
```
...
descriptive_stat=cbind(df_t[,1],mean_df,sd_df,median_df)
...
group=list()
  ...
  #current_team=subset(select(current_team,2:ncol(current_team)))
